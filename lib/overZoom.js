...
async function overZoom(db, t) {
  ...
  const tileClipMatrixArray = []

  for (let i = 0; i < ZOOMFACTOR; i++ ) {
    this._tile = (!i) ? toMercatorTile(tile) : tileClipMatrixArray[i-1].tile
    const obj = {
      matrix: getTileClipMatrix(this._tile),
      tile: tilebelt.getParent(this._tile)
    };
    tileClipMatrixArray.push(obj)
  }

  console.log(tileClipMatrixArray[zoomFactor])

  const tileData = getOriginTileData(db, originTile);

  switch (zoomFactor) {
    case 1 : return expandAndClipTile(tileData, tileClipMatrixArray[zoomFactor - 1].matrix);
    case 2 : 
      const zf2 = await expandAndClipTile(tileData, tileClipMatrixArray[zoomFactor - 1].matrix);
      return expandAndClipTile(zf2.buffer, tileClipMatrixArray[zoomFactor - 2].matrix, zf2.metadata)
    case 3 : 
