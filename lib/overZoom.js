 ...
 */
const getTilePosition = (t, level) => {

  switch (level) {
    case 2 : this._tile = tilebelt.getParent(toMercatorTile(t));
    case 3 : this._tile = tilebelt.getParent(tilebelt.getParent(toMercatorTile(t)));
    default : this._tile = toMercatorTile(t)
  }
...
async function overZoom(db, tile) {
  ...
  if (![1,2].includes(tile[0] - maxNativeZoom)) return false

  const zoomFactor = tile[0] - maxNativeZoom;

  const originTile = getOriginTile(zoomFactor, tile);

  const clipMatrix = getTilePosition(tile)
  const parentClipMatrix = getParentTilePosition(tile, 2);
  const tileData = getOriginTileData(db, originTile);

  switch (zoomFactor) {
    case 1 : return expandAndClipTile(tileData, clipMatrix);
    case 2 : 
      const data = await expandAndClipTile(tileData, parentClipMatrix);
      return expandAndClipTile(data.buffer, clipMatrix, data.metadata)
    case 3 : return false;
