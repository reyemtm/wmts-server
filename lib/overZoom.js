...
const checkRasterFormatAndMaxzoom = (db) => {
  try {
    ...
    throw new Error("Error in OverZoom function: could not query database. " + error)      
  }
}
...
const expandAndClipTile = async (tileData, matrix, meta) => {
  ...
  // } : meta
  // if (!meta) {
  //   const jimp = await Jimp.read(tileData);
  //   metadata.format = jimp.getMIME().replace("image/", "")
  // }

  //TODO attempted to use jpeg as input and convert white background to transparent but this takes way too long
  // const now = Date.now()
  // const bitmap = await replaceColor({
  //   image: tileData,
  //   colors: {
  //     type: 'hex',
  //     targetColor: "#ffffff",
  ...
  // });
  // const a = Date.now();
  // const transparent = await bitmap.getBufferAsync("image/png");
  // const b = Date.now()

  // Jimp is too slow on the Synology NAS to bother
  // const jimp = await Jimp.read(tileData);
  // const resized = jimp.resize(512, 512, Jimp.RESIZE_BEZIER);
  // const cropped = resized.crop(matrix.left, matrix.top, 256, 256)
  // const q = (metadata.format === "png") ? cropped.deflateLevel(9) : cropped.quality(50)
  // const buffer = await q.getBufferAsync(q.getMIME())
